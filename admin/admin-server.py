...
.toolbar{display:flex;gap:10px;flex-wrap:wrap;margin:20px 0}input,button{font:inherit;border-radius:9px;border:1px solid var(--line);padding:10px 13px}input{background:#0b0e13;color:var(--text);min-width:220px}button{background:var(--blue);color:#07101c;border:0;font-weight:700;cursor:pointer}button.secondary{background:transparent;color:var(--text);border:1px solid var(--line)}label{display:flex;align-items:center;gap:7px;color:var(--muted);padding:10px 0}.pill{color:var(--muted);font-size:.85rem}
...
<div class="toolbar"><input id="token" type="password" placeholder="Admin token (if configured)" autocomplete="current-password"><input id="filter" type="search" placeholder="Search device, MAC, or IP" oninput="render()"><button onclick="load()">Refresh now</button><label><input id="live" type="checkbox" checked onchange="toggleLive()"> live refresh</label><span class="pill" id="updated">—</span></div>
...
<script>
let clients=[];let timer=null;
async function load(){const token=document.querySelector('#token').value;const r=await fetch('/api/clients',{headers:token?{Authorization:'Bearer '+token}:{}});const data=await r.json();if(!r.ok){document.querySelector('#rows').innerHTML='<tr><td colspan="5" class="empty">'+(data.error||'Unable to load clients')+'</td></tr>';return}clients=data.clients||[];document.querySelector('#updated').textContent='Updated '+new Date().toLocaleTimeString();document.querySelector('#total').textContent=clients.length;document.querySelector('#auth').textContent=clients.filter(x=>x.state==='authenticated').length;document.querySelector('#trusted').textContent=clients.filter(x=>x.state==='trusted').length;render()}
function render(){const q=document.querySelector('#filter').value.toLowerCase();const list=clients.filter(x=>[x.hostname,x.mac,x.ip,x.state].join(' ').toLowerCase().includes(q));document.querySelector('#rows').innerHTML=list.length?list.map(x=>{const action=x.state==='trusted'?'untrust':x.state==='authenticated'?'deauth':'trust';const label=x.state==='trusted'?'Revoke':x.state==='authenticated'?'Disconnect':'Trust';return `<tr><td>${x.hostname||'Unknown'}<small>${x.mac||''}</small></td><td>${x.ip||'—'}<small>lease ${x.expires||'—'}</small></td><td><span class="badge ${x.state==='trusted'?'warn':''}">${x.state}</span></td><td><b>${x.sessiontimeout||'—'} min</b><small>up ${x.uploadrate||0} / down ${x.downloadrate||0} kb/s<br>used ${x.uploadused||0} / ${x.downloadused||0} kB</small></td><td><button class="secondary" onclick="act('${x.mac}','${action}')">${label}</button><button class="secondary" onclick="limits('${x.mac}')">Set limits</button></td></tr>`}).join(''):'<tr><td colspan="5" class="empty">No matching clients.</td></tr>'}
...
function toggleLive(){if(timer)clearInterval(timer);timer=document.querySelector('#live').checked?setInterval(load,5000):null}
